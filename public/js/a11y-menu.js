...
<div class="uc-a11y__row">
  <span>Lecture </span>
  <div class="uc-a11y__btnGroup">
    <button class="uc-a11y__btn" type="button" data-tts="toggle" id="uc-tts-toggle">Traduction</button>
  </div>
</div>

<label class="uc-a11y__toggle">
  <input type="checkbox" data-tts="translate" id="uc-tts-translate">
  <span>Traduire avant lecture</span>
</label>

<div class="uc-a11y__row">
  <span>Langue</span>
  <div class="uc-a11y__btnGroup">
    <select class="uc-a11y__btn" data-tts="lang" id="uc-tts-lang">
...
<div class="uc-a11y__mini">
  Astuce : cliquez une fois sur la page pour activer la voix (obligatoire sur certains navigateurs).
</div>
    ...
    // ---- TTS wiring (requires tts-hover.js) ----
const ttsToggleBtn = wrap.querySelector('#uc-tts-toggle');
const ttsTranslate = wrap.querySelector('#uc-tts-translate');
const ttsLang = wrap.querySelector('#uc-tts-lang');

function syncTtsUI() {
  const api = window.ttsHover;
  if (!api?.getState) {
    // script not loaded
    if (ttsToggleBtn) ttsToggleBtn.textContent = 'TTS (script manquant)';
    return;
  }
  const s = api.getState();
  if (ttsToggleBtn) ttsToggleBtn.textContent = s.enabled ? 'TTS ON' : 'TTS OFF';
  if (ttsTranslate) ttsTranslate.checked = !!s.translateOn;
  if (ttsLang) ttsLang.value = s.targetLang || 'en';
}
...
ttsToggleBtn?.addEventListener('click', () => {
  ...
  const on = api.toggle();
  ttsToggleBtn.textContent = on ? 'TTS ON' : 'TTS OFF';
});

// Translate switch
ttsTranslate?.addEventListener('change', (e) => {
  const api = window.ttsHover;
  if (!api?.setTranslate) return;
  api.setTranslate(e.target.checked);
});

// Language select
ttsLang?.addEventListener('change', (e) => {
  const api = window.ttsHover;
  if (!api?.setTargetLang) return;
  api.setTargetLang(e.target.value);
});

// Sync when menu loads
syncTtsUI();
